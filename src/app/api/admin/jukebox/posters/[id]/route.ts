...
export async function PATCH(req: NextRequest, ctx: { params: { id: string } }) {
  ...
  if (error) {
    console.error('[posters PATCH]', error.message);
    return NextResponse.json(
...
export async function DELETE(req: NextRequest, ctx: { params: { id: string } }) {
  ...
  if (dbErr) {
    console.error('[posters DELETE] db error:', dbErr.message);
    return NextResponse.json(
  ...
  // Best-effort storage cleanup — don't fail the response if this errors.
  await sb.storage.from(BUCKET).remove([row.storage_path]).catch((e: unknown) => {
    console.error('[posters DELETE] storage remove error:', e);
  });
